...
ea=root/'mt5/EVE_Twelve_Data_Fixed_Ladder_v2.61.mq5'
...
required_ea=[
 '#property version   "2.61"','2907202622','EVEL261','FIXED_LADDER_FLIGHT_RECORDER',
 ...
 'InpProfitTargetEnabledAtStart     = true','runtime_profit_target_enabled','runtime_profit_target_money','campaign_profit_target_enabled','campaign_profit_target_money','SaveLocalSettings','LoadLocalSettings',
 ...
 'InpBasketPeakProtectionAtStart    = true','InpBasketPeakActivationMoney      = 4.00','runtime_basket_peak_protection_enabled','campaign_peak_protection_enabled','BASKET PEAK PROTECTION FLOOR','basketPeakProtectionEnabled','basketPeakProtectionFloor',
 ...
 'CAMPAIGN PROFIT TARGET','UNIQUE_POSITION_IDENTIFIER','eventSequence','ReconcileCampaignCountersFromHistory','ReportingCampaignId','TELEMETRY_QUEUE_SIZE 2048','ladderLevel','campaignBuyBulletsFired','INITIAL STOP LOSS','BE PROTECTED STOP','MaybeQueueReplaySnapshot','QueueLadderReport','/api/ea/replay','/api/ea/ladder','/api/ea/bullet-protection',
...
for item in ['FIXED 8×8 LADDER FLIGHT RECORDER','profitTargetEnabled','dailyLossEnabled','basketPeakProtectionEnabled','basketPeakActivationMoney','basketPeakGivebackMoney','/api/daily-loss/reset','/api/settings','/api/replay/','ladders','replay','protections','eve-fixed-ladder-${collection}.csv','auditCampaign','campaignTimeline']:
    if item not in server+ui: errors.append(f'Missing Railway/UI element: {item}')
for item in ["version: '2.6.1'","mode: 'FIXED_LADDER_FLIGHT_RECORDER_DEMO'","BULLET_DATA_NAMESPACE || 'v220'"]:
    if item not in cfg: errors.append(f'Missing v2.61 config: {item}')
if errors:
