...
#property strict
#property description "XAUUSD fixed 8x8 ladder: equal 0.01 bullets, halfway BE protection, newest-bullet sentinel, optional campaign profit target and full flight-recorder telemetry."
...
input string InpOrderCommentPrefix             = "EVEL220";
...
input group "Legacy optional protection inputs (not used in v2.20)"
...
// Legacy names retained for source compatibility. They do not gate entries in v2.20.
...
int OnInit()
{
   ...
   {
      Print("EVE Fixed Ladder v2.20 cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Alert("EVE Fixed Ladder v2.20 requires a HEDGING demo account.");
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Fixed Ladder v2.20 failed to create ATR handle. Error ", GetLastError());
      return INIT_FAILED;
   }
   ...
   last_event = "v2.20 ready: fixed 8x8 ladder, halfway BE, newest-bullet sentinel and flight recorder";
   Print(last_event);
                        ...
                        const MqlTradeResult &result)
{
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      direction_leg_count = campaign_entries;
      campaign_max_positions = (int)MathMax(campaign_max_positions, CountOurPositions());
      campaign_side = buy_leg_count > 0 && sell_leg_count > 0 ? "MIXED" : (buy_leg_count > 0 ? "BUY" : "SELL");
      ...
      int bullet_number = side == "BUY" ? buy_leg_count : sell_leg_count;
      double initial_sl = side == "BUY" ? price - FallbackDistance() : price + FallbackDistance();
      if(PositionSelectByTicket(trans.position)) initial_sl = PositionGetDouble(POSITION_SL);
      RegisterBullet(position_id, trans.position, side, bullet_number, price, initial_sl);
      ...
      QueueLegReportDetailed("OPEN", side, trans.position, position_id, bullet_number, volume, price, initial_sl, initial_sl, false, 0, 0.0, 0.0, 0.0,
      ...
      QueueSignalReport(first_entry ? "SCOUT_FIRED" : "BULLET_FIRED", side, price,
                        StringFormat("Fixed ladder bullet %d fired; both original ladders remain fixed", bullet_number));
      last_event = StringFormat("%s bullet %d fired at %.2f; newest bullet is now sentinel", side, bullet_number, price);
   }
   else if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_OUT_BY)
   {
      ...
      QueueLegReportDetailed("CLOSE", position_side, trans.position, position_id, bullet_number, volume, price, initial_sl, final_sl, be_active, time_to_be, mfe, mae, profit, close_reason);

      bool was_newest = position_id > 0 && position_id == newest_position_id;
      if(was_newest && InpCloseBasketOnNewestLegSL && reason == DEAL_REASON_SL)
      {
         newest_leg_sl_exit_detected = true;
         basket_close_requested = true;
         basket_close_reason = be_active ? "NEWEST BULLET BE STOP - CLOSE FULL CAMPAIGN" : "NEWEST BULLET FAILED BEFORE HALFWAY - CLOSE FULL CAMPAIGN";
         campaign_exit_reason = basket_close_reason;
         adding_stopped = true;
         last_event = StringFormat("Newest %s bullet %d hit %s; closing every position and pending order", position_side, bullet_number, be_active ? "BE stop" : "original stop");
      }
      else
...
void RecoverCampaignFromLivePositions()
{
   ...
   UpdateNewestLegStats();
   last_event = "Recovered fixed-ladder campaign after restart; both original sides remain independent";
}
...
{
   if(campaign_id == "") campaign_id = BuildCampaignId(side);
   ResetBulletTracking();
   campaign_side = side;
...
bool PlaceBulletStop(string side, string role, int leg_number, string reason)
{
   ...
   if(!accepted)
   {
      ...
      Print("EVE Fixed Ladder v2.20 ", last_event, " MQL=", GetLastError());
...
bool CancelAllPending(string reason)
{
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      Print("EVE Fixed Ladder v2.20 ", last_event, " reason=", reason, " MQL=", GetLastError());
   }
...
void ManageIndividualProtection(const MqlTick &tick)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      if(submitted && IsAcceptedTradeRetcode(code))
      {
         ...
         PrintFormat("EVE Fixed Ladder v2.20 BE modify %I64u rejected %u %s MQL=%d", ticket, code, trade.ResultRetcodeDescription(), GetLastError());
   }
}
...
bool CloseAllPositions(string reason)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      {
         all_closed = false;
         PrintFormat("EVE Fixed Ladder v2.20 close %I64u failed %u %s MQL=%d", ticket, code, trade.ResultRetcodeDescription(), GetLastError());
      }
   }
...
void QueueBulletProtection(ulong position_id, ulong ticket, int bullet_number, string side, double entry, double new_sl, double progress)
{
   ...
   string json=StringFormat("{\"id\":\"%I64d-%I64u-BE\",\"campaignId\":\"%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"at\":%I64d,\"action\":\"BE_ACTIVATED\",\"side\":\"%s\",\"bulletNumber\":%d,\"ticket\":\"%I64u\",\"positionId\":\"%I64u\",\"entryPrice\":%.5f,\"newSl\":%.5f,\"progressPrice\":%.5f,\"triggerPrice\":%.5f,\"bufferPrice\":%.5f}",
      now_ms,ticket,JsonEscape(campaign_id),AccountInfoInteger(ACCOUNT_LOGIN),JsonEscape(trade_symbol),InpMagicNumber,now_ms,
...
void MaybeQueueReplaySnapshot()
{
   ...
   long now_ms=(long)TimeCurrent()*1000;
   string json=StringFormat("{\"id\":\"%I64d-%I64u\",\"campaignId\":\"%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"at\":%I64d,\"bid\":%.5f,\"ask\":%.5f,\"spreadPoints\":%.1f,\"floatingProfit\":%.2f,\"peakProfit\":%.2f,\"positions\":%d,\"pending\":%d,\"newestTicket\":\"%I64u\",\"profitTargetEnabled\":%s,\"profitTargetMoney\":%.2f,\"positionState\":%s,\"pendingState\":%s}",
      now_ms,GetTickCount64(),JsonEscape(campaign_id),AccountInfoInteger(ACCOUNT_LOGIN),JsonEscape(trade_symbol),now_ms,tick.bid,tick.ask,
      CurrentSpreadPoints(),BasketFloatingProfit(),campaign_peak_floating,CountOurPositions(),CountOurPending(),newest_ticket,
      runtime_profit_target_enabled?"true":"false",runtime_profit_target_money,PositionReplayJson(),PendingReplayJson());
...
void QueueLadderReport(string reason)
{
   ...
   buys+="]"; sells+="]";
   long now_ms=(long)TimeCurrent()*1000;
   string json=StringFormat("{\"id\":\"%s\",\"campaignId\":\"%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"at\":%I64d,\"anchorPrice\":%.5f,\"levelsPerSide\":%d,\"spacing\":%.5f,\"lot\":%.2f,\"fallback\":%.5f,\"beTrigger\":%.5f,\"beBuffer\":%.5f,\"buyPrices\":%s,\"sellPrices\":%s,\"reason\":\"%s\"}",
      JsonEscape(campaign_id),JsonEscape(campaign_id),AccountInfoInteger(ACCOUNT_LOGIN),JsonEscape(trade_symbol),now_ms,ladder_anchor_price,levels,
...
{
   long now_ms=(long)TimeCurrent()*1000;
   string json=StringFormat("{\"id\":\"%I64d-%s-%d\",\"campaignId\":\"%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"at\":%I64d,\"action\":\"%s\",\"side\":\"%s\",\"campaignEntry\":%d,\"price\":%.5f,\"fallbackDistance\":%.5f,\"bulletSpacing\":%.5f,\"beTrigger\":%.5f,\"reason\":\"%s\"}",
      now_ms,JsonEscape(action),campaign_entries,JsonEscape(campaign_id),AccountInfoInteger(ACCOUNT_LOGIN),JsonEscape(trade_symbol),InpMagicNumber,
                            ...
                            int time_to_be_seconds, double mfe_price, double mae_price, double net_profit, string reason)
{
   ...
   string json = StringFormat(
      "{\"id\":\"%I64d-%I64u-%s\",\"campaignId\":\"%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"dealTime\":%I64d,\"action\":\"%s\",\"side\":\"%s\",\"bulletNumber\":%d,\"ticket\":\"%I64u\",\"positionId\":\"%I64u\",\"volume\":%.2f,\"price\":%.5f,\"initialSl\":%.5f,\"finalSl\":%.5f,\"beActivated\":%s,\"timeToBeSeconds\":%d,\"mfePrice\":%.5f,\"maePrice\":%.5f,\"netProfit\":%.2f,\"reason\":\"%s\"}",
      now_ms, ticket, JsonEscape(action), JsonEscape(campaign_id), AccountInfoInteger(ACCOUNT_LOGIN), JsonEscape(trade_symbol), InpMagicNumber,
      now_ms, JsonEscape(action), JsonEscape(side), bullet_number, ticket, position_id, volume, price, initial_sl, final_sl,
      be_active ? "true" : "false", time_to_be_seconds, mfe_price, mae_price, net_profit, JsonEscape(reason));
...
void QueueOrderReport(string action, string role, string order_type, ulong ticket, double volume, double price, string reason)
{
   long now_ms = (long)TimeCurrent() * 1000;
   string json = StringFormat(
      "{\"id\":\"%I64d-%I64u-%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"at\":%I64d,\"action\":\"%s\",\"role\":\"%s\",\"orderType\":\"%s\",\"ticket\":\"%I64u\",\"volume\":%.2f,\"price\":%.5f,\"reason\":\"%s\"}",
...
{
   long now_ms=(long)TimeCurrent()*1000;
   string json=StringFormat("{\"id\":\"%I64d-%I64u\",\"campaignId\":\"%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"side\":\"%s\",\"basketProfit\":%.2f,\"peakBasketProfit\":%.2f,\"newestTicket\":\"%I64u\",\"newestProfit\":%.2f,\"newestPeak\":%.2f,\"profitTargetEnabled\":%s,\"profitTargetMoney\":%.2f,\"reason\":\"%s\",\"at\":%I64d}",
      now_ms,newest_ticket,JsonEscape(campaign_id),AccountInfoInteger(ACCOUNT_LOGIN),JsonEscape(trade_symbol),InpMagicNumber,JsonEscape(campaign_side),
...
void MaybeQueueScanReport()
{
   ...
   long now_ms = (long)TimeCurrent() * 1000;
   string json = StringFormat(
      "{\"id\":\"%I64d\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"momentumState\":\"%s\",\"watchDirection\":\"%s\",\"buyScore\":%d,\"sellScore\":%d,\"velocity1s\":%.5f,\"velocity3s\":%.5f,\"velocity10s\":%.5f,\"tickRateRatio\":%.3f,\"acceleration\":%.3f,\"bodyAtr\":%.3f,\"blockReason\":\"%s\",\"engineState\":\"%s\",\"buyPositions\":%d,\"sellPositions\":%d,\"pendingOrders\":%d,\"activeBullets\":%d}",
...
void SendHeartbeat()
{
   ...
   string json = StringFormat(
      "{\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"magic\":\"%I64u\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"bid\":%.5f,\"ask\":%.5f,\"spreadPoints\":%.1f,\"terminalConnected\":%s,\"algoAllowed\":%s,\"autonomous\":%s,\"emergency\":%s,\"engineState\":\"%s\",\"campaignId\":\"%s\",\"campaignCurrentSide\":\"%s\",\"campaignBuyLegs\":%d,\"campaignSellLegs\":%d,\"positionCount\":%d,\"pendingCount\":%d,\"totalLots\":%.2f,\"floatingProfit\":%.2f,\"peakBasketProfit\":%.2f,\"basketMae\":%.2f,\"positionsOpened\":%d,\"newestTicket\":\"%I64u\",\"newestLegProfit\":%.2f,\"newestLegPeak\":%.2f,\"newestLegAgeSeconds\":%d,\"profitTargetEnabled\":%s,\"profitTargetMoney\":%.2f,\"beTriggerPrice\":%.3f,\"beBufferPrice\":%.3f,\"ladderAnchor\":%.5f,\"gridSpacing\":%.3f,\"fallbackDistance\":%.3f,\"telemetryQueueDepth\":%d,\"lastHttpStatus\":\"%s\",\"lastEvent\":\"%s\",\"consumedCommandId\":\"%I64d\",\"lastCommandResult\":\"%s\"}",
      ...
      JsonEscape(EngineStateText()), JsonEscape(campaign_id), JsonEscape(campaign_side), buy_leg_count, sell_leg_count,
      positions, CountOurPending(), OurTotalLots(), BasketFloatingProfit(), campaign_peak_floating, campaign_worst_floating,
      campaign_entries, newest_ticket, newest_leg_current_profit, newest_leg_peak_profit, newest_age,
      runtime_profit_target_enabled ? "true" : "false", runtime_profit_target_money,
      InpBreakEvenTriggerPrice, InpBreakEvenBufferPrice, ladder_anchor_price, BulletSpacing(), FallbackDistance(), telemetry_count,
...
void PollRailway()
{
   ...
   ResetLastError();
   int status = WebRequest("GET", url, "Accept: text/plain\r\nConnection: close\r\n", InpWebTimeoutMilliseconds, request_data, result, response_headers);
   if(status != 200)
   {
   ...
   if(incoming_settings_version > runtime_settings_version)
   {
      ...
      last_event = runtime_profit_target_enabled
         ? StringFormat("Dashboard target applied: bank %.2f then rearm", runtime_profit_target_money)
         : "Dashboard target OFF: natural sentinel mode";
   }
...
void RegisterHttpFailure(string endpoint, int status, string response)
{
   ...
   Print("EVE Fixed Ladder v2.20 Railway ", last_http_status, " response=", response);
}
...
void QueueBasketReport(string side, double realised)
{
   ...
   queued_basket_json=StringFormat("{\"id\":\"%s\",\"campaignId\":\"%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.20\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"status\":\"CLOSED\",\"side\":\"%s\",\"entryTime\":%I64d,\"exitTime\":%I64d,\"durationSeconds\":%d,\"positionsOpened\":%d,\"buyBullets\":%d,\"sellBullets\":%d,\"lotPerLeg\":%.2f,\"anchorPrice\":%.5f,\"gridSpacing\":%.5f,\"fallbackDistance\":%.5f,\"beTriggerPrice\":%.5f,\"beBufferPrice\":%.5f,\"profitTargetEnabled\":%s,\"profitTargetMoney\":%.2f,\"netProfit\":%.2f,\"peakBasketProfit\":%.2f,\"mae\":%.2f,\"profitGiveback\":%.2f,\"exitReason\":\"%s\"}",
      JsonEscape(campaign_id),JsonEscape(campaign_id),AccountInfoInteger(ACCOUNT_LOGIN),JsonEscape(trade_symbol),InpMagicNumber,JsonEscape(side),
      started_ms,ended_ms,campaign_started_at>0?(int)(TimeCurrent()-campaign_started_at):0,campaign_entries,buy_leg_count,sell_leg_count,
      EffectiveLot(),ladder_anchor_price,BulletSpacing(),FallbackDistance(),InpBreakEvenTriggerPrice,InpBreakEvenBufferPrice,
      runtime_profit_target_enabled?"true":"false",runtime_profit_target_money,realised,campaign_peak_floating,campaign_worst_floating,
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX + "TITLE", 12, 18, "EVE BULLET STORM v2.20", 12);
