...
input ulong  InpMagicNumber                    = 2807202601;
input string InpOrderCommentPrefix             = "EVETD100";
...
int OnInit()
{
   ...
   {
      Print("EVE TD v1.00 cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Alert("EVE TD v1.00 requires a HEDGING demo account.");
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE TD v1.00 failed to create ATR handle. Error ", GetLastError());
      return INIT_FAILED;
   }
   ...
   last_event = "v1.00 ready: Twelve Data decision engine with MT5 protected execution";
   Print(last_event);
...
{
   if(remote_decision_valid_until <= 0) return false;
   long now_ms = (long)TimeCurrent() * 1000;
   return now_ms <= remote_decision_valid_until && now_ms + (long)MathMax(1, InpDecisionMaximumAgeSeconds) * 1000 >= remote_decision_valid_until;
}
...
bool PlaceInitialScout(string side)
{
   ...
   if(!accepted)
   {
      ...
      Print("EVE TD v1.00 ", last_event, " MQL=", GetLastError());
...
bool CancelAllPending(string reason)
{
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      Print("EVE TD v1.00 ", last_event, " reason=", reason, " MQL=", GetLastError());
   }
...
void ManageIndividualProtection(const MqlTick &tick)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      if(!submitted || !IsAcceptedTradeRetcode(code))
      {
         PrintFormat("EVE TD v1.00 position %I64u protection modify rejected %u %s MQL=%d", ticket, code, trade.ResultRetcodeDescription(), GetLastError());
      }
   }
}
...
bool CloseAllPositions(string reason)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      {
         all_closed = false;
         PrintFormat("EVE TD v1.00 close %I64u failed %u %s MQL=%d", ticket, code, trade.ResultRetcodeDescription(), GetLastError());
      }
   }
...
void QueueLegReport(string action, string side, ulong ticket, ulong position_id, double volume, double price, double net_profit, string reason)
{
   ...
   string json = StringFormat(
      "{\"id\":\"%I64d-%I64u-%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"1.00\",\"strategy\":\"TWELVE_DATA_CONTEXT_SCOUT_SENTINEL_TRADER\",\"magic\":\"%I64u\",\"dealTime\":%I64d,\"action\":\"%s\",\"side\":\"%s\",\"ticket\":\"%I64u\",\"positionId\":\"%I64u\",\"volume\":%.2f,\"price\":%.5f,\"netProfit\":%.2f,\"reason\":\"%s\"}",
...
void QueueOrderReport(string action, string role, string order_type, ulong ticket, double volume, double price, string reason)
{
   long now_ms = (long)TimeCurrent() * 1000;
   string json = StringFormat(
      "{\"id\":\"%I64d-%I64u-%s\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"1.00\",\"strategy\":\"TWELVE_DATA_CONTEXT_SCOUT_SENTINEL_TRADER\",\"magic\":\"%I64u\",\"at\":%I64d,\"action\":\"%s\",\"role\":\"%s\",\"orderType\":\"%s\",\"ticket\":\"%I64u\",\"volume\":%.2f,\"price\":%.5f,\"reason\":\"%s\"}",
      now_ms, ticket, JsonEscape(action), AccountInfoInteger(ACCOUNT_LOGIN), JsonEscape(trade_symbol), InpMagicNumber,
...
void QueueBankDecision(string reason, double basket_profit)
{
   ...
   long now_ms = (long)TimeCurrent() * 1000;
   string json = StringFormat(
      "{\"id\":\"%I64d-%I64u\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"1.00\",\"strategy\":\"TWELVE_DATA_CONTEXT_SCOUT_SENTINEL_TRADER\",\"magic\":\"%I64u\",\"side\":\"%s\",\"basketProfit\":%.2f,\"peakBasketProfit\":%.2f,\"newestTicket\":\"%I64u\",\"newestProfit\":%.2f,\"newestPeak\":%.2f,\"sameScore\":%d,\"oppositeScore\":%d,\"momentumState\":\"%s\",\"reason\":\"%s\",\"at\":%I64d}",
...
void MaybeQueueScanReport()
{
   ...
   long now_ms = (long)TimeCurrent() * 1000;
   string json = StringFormat(
      "{\"id\":\"%I64d\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"1.00\",\"strategy\":\"TWELVE_DATA_CONTEXT_SCOUT_SENTINEL_TRADER\",\"magic\":\"%I64u\",\"decisionId\":\"%s\",\"decisionAction\":\"%s\",\"decisionDirection\":\"%s\",\"decisionQuality\":%.2f,\"buyQuality\":%.2f,\"sellQuality\":%.2f,\"addAllowed\":%s,\"regime\":\"%s\",\"m5\":\"%s\",\"m15\":\"%s\",\"h1\":\"%s\",\"localMomentumState\":\"%s\",\"localBuyScore\":%d,\"localSellScore\":%d,\"velocity1s\":%.5f,\"velocity3s\":%.5f,\"velocity10s\":%.5f,\"tickRateRatio\":%.3f,\"acceleration\":%.3f,\"bodyAtr\":%.3f,\"blockReason\":\"%s\"}",
...
void RegisterHttpFailure(string endpoint, int status, string response)
{
   ...
   Print("EVE TD v1.00 Railway ", last_http_status, " response=", response);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX + "TITLE", 12, 18, "EVE TWELVE DATA MOMENTUM TRADER v1.00", 12);
