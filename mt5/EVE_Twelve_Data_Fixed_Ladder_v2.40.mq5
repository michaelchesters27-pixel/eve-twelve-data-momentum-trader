...
#property strict
#property description "XAUUSD fixed 8x8 ladder: Bullet 1 onward earns halfway BE, protected stops close one bullet, newest initial-SL failure closes the campaign, selectable profit and daily-loss controls."
...
input string InpOrderCommentPrefix             = "EVEL240";
...
input double InpMaximumDailyLossPercent        = 0.00; // legacy compatibility only; dashboard money limit controls v2.40
...
input group "Legacy optional protection inputs (not used in v2.40)"
...
// Legacy names retained for source compatibility. They do not gate entries in v2.40.
...
int OnInit()
{
   ...
   {
      Print("EVE Fixed Ladder v2.40 cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Alert("EVE Fixed Ladder v2.40 requires a HEDGING demo account.");
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Fixed Ladder v2.40 failed to create ATR handle. Error ", GetLastError());
      return INIT_FAILED;
   }
   ...
   last_event = "v2.40 ready: Bullet 1 onward earns BE at halfway, protected BE exits close only that bullet, dashboard daily-loss control active";
   Print(last_event);
                        ...
                        const MqlTradeResult &result)
{
   ...
   {
      PrintFormat("EVE Fixed Ladder v2.40 ignored duplicate deal callback %I64u", trans.deal);
      return;
   }
   ...
   else if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_OUT_BY)
   {
      ...
      if(reason == DEAL_REASON_SL)
         close_reason = be_active ? "BE PROTECTED STOP - BULLET ONLY" : "INITIAL STOP LOSS";
      ...
      if(newest_failed_before_halfway)
      {
         ...
         basket_close_reason = "NEWEST BULLET FAILED BEFORE HALFWAY - CLOSE FULL CAMPAIGN";
      ...
      direction_leg_count = CountOurPositions();
      if(newest_failed_before_halfway)
         last_event = StringFormat("Newest %s bullet %d failed before halfway; closing every position and pending order", position_side, bullet_number);
...
bool PlaceBulletStop(string side, string role, int leg_number, string reason)
{
   ...
   if(!accepted)
   {
      ...
      Print("EVE Fixed Ladder v2.40 ", last_event, " MQL=", GetLastError());
...
bool CancelAllPending(string reason)
{
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      Print("EVE Fixed Ladder v2.40 ", last_event, " reason=", reason, " MQL=", GetLastError());
   }
...
void ManageIndividualProtection(const MqlTick &tick)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      if(submitted && IsAcceptedTradeRetcode(code))
      {
         ...
         PrintFormat("EVE Fixed Ladder v2.40 BE modify %I64u rejected %u %s MQL=%d", ticket, code, trade.ResultRetcodeDescription(), GetLastError());
   }
}
...
bool CloseAllPositions(string reason)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      {
         all_closed = false;
         PrintFormat("EVE Fixed Ladder v2.40 close %I64u failed %u %s MQL=%d", ticket, code, trade.ResultRetcodeDescription(), GetLastError());
      }
   }
...
void QueueLadderReport(string reason)
{
   ...
   long now_ms=(long)TimeCurrent()*1000;
   long seq=NextCampaignEventSequence();
   string json=StringFormat("{\"id\":\"%s\",\"campaignId\":\"%s\",\"eventSequence\":%I64d,\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.40\",\"at\":%I64d,\"anchorPrice\":%.5f,\"levelsPerSide\":%d,\"spacing\":%.5f,\"lot\":%.2f,\"fallback\":%.5f,\"beTrigger\":%.5f,\"beBuffer\":%.5f,\"buyPrices\":%s,\"sellPrices\":%s,\"reason\":\"%s\"}",
...
void QueueSignalReport(string action, string side, double price, string reason)
{
   long now_ms=(long)TimeCurrent()*1000;
   long seq=NextCampaignEventSequence();
   string json=StringFormat("{\"id\":\"%I64d-%s-%d\",\"campaignId\":\"%s\",\"eventSequence\":%I64d,\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.40\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"at\":%I64d,\"action\":\"%s\",\"side\":\"%s\",\"campaignEntry\":%d,\"price\":%.5f,\"fallbackDistance\":%.5f,\"bulletSpacing\":%.5f,\"beTrigger\":%.5f,\"reason\":\"%s\"}",
...
void QueueOrderReport(string action, string role, string order_type, ulong ticket, double volume, double price, string reason)
{
   ...
   long seq = NextCampaignEventSequence();
   string json = StringFormat(
      "{\"id\":\"%I64d-%I64u-%s\",\"campaignId\":\"%s\",\"eventSequence\":%I64d,\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.40\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"at\":%I64d,\"action\":\"%s\",\"role\":\"%s\",\"orderType\":\"%s\",\"ticket\":\"%I64u\",\"volume\":%.2f,\"price\":%.5f,\"reason\":\"%s\"}",
      now_ms, ticket, JsonEscape(action), JsonEscape(campaign_id), seq, AccountInfoInteger(ACCOUNT_LOGIN), JsonEscape(trade_symbol), InpMagicNumber,
...
void QueueBankDecision(string reason, double basket_profit)
{
   long now_ms=(long)TimeCurrent()*1000;
   long seq=NextCampaignEventSequence();
   string json=StringFormat("{\"id\":\"%I64d-%I64u\",\"campaignId\":\"%s\",\"eventSequence\":%I64d,\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.40\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"side\":\"%s\",\"basketProfit\":%.2f,\"peakBasketProfit\":%.2f,\"uniqueBulletsFired\":%d,\"newestTicket\":\"%I64u\",\"newestProfit\":%.2f,\"newestPeak\":%.2f,\"profitTargetEnabled\":%s,\"profitTargetMoney\":%.2f,\"reason\":\"%s\",\"at\":%I64d}",
...
void MaybeQueueScanReport()
{
   ...
   long now_ms = (long)TimeCurrent() * 1000;
   string json = StringFormat(
      "{\"id\":\"%I64d\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"2.40\",\"strategy\":\"FIXED_LADDER_FLIGHT_RECORDER\",\"magic\":\"%I64u\",\"momentumState\":\"%s\",\"watchDirection\":\"%s\",\"buyScore\":%d,\"sellScore\":%d,\"velocity1s\":%.5f,\"velocity3s\":%.5f,\"velocity10s\":%.5f,\"tickRateRatio\":%.3f,\"acceleration\":%.3f,\"bodyAtr\":%.3f,\"blockReason\":\"%s\",\"engineState\":\"%s\",\"buyPositions\":%d,\"sellPositions\":%d,\"pendingOrders\":%d,\"activeBullets\":%d}",
...
void RegisterHttpFailure(string endpoint, int status, string response)
{
   ...
   Print("EVE Fixed Ladder v2.40 Railway ", last_http_status, " response=", response);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX + "TITLE", 12, 18, "EVE FIXED LADDER v2.40", 12);
...
void UpdatePanel()
{
   ...
   SetLabel(PANEL_PREFIX + "CAMPAIGN", StringFormat("%s | pos %d | pend %d | P/L %.2f | daily %s %.2f", campaign_side, CountOurPositions(), CountOurPending(), BasketFloatingProfit(), runtime_daily_loss_enabled ? "ON" : "OFF", runtime_daily_loss_money));
